...
- Publish readiness should be simple before it is flashy: compact summary tiles, a subdued action rail, one readable session automation panel, and one compact open-task panel. Avoid side rails, score cards, or multi-column compositions when the card does not have enough width.
