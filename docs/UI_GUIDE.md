...
- Publish blockers are system-readiness items only: sessions and calendar plans exist, required session communications are created/scheduled, and genuine manual session tasks are cleared. Materials are optional and never block publication. Registration, roster, payment, and document follow-ups remain operational work but must not block a cohort from becoming Published.
- Moving a cohort back to `Draft` is the supported delivery-off switch. It must pause unsent/scheduled app emails while preserving sent history; calendar cancellations or attendee updates remain separate intentional actions.
- Publish readiness must summarize blockers, not repeat the session checklist. Show compact summary tiles and a subdued action rail only; the Sessions checklist is the single detailed source for per-session calendar/email/material status.
- Publish readiness should be simple before it is flashy. Avoid side rails, score cards, separate automation panels, or duplicate task lists on Overview when the same information exists in Sessions or a dedicated tab.
...
- Editing a session with a linked Google event creates a persistent pending change. `Apply Changes` updates all affected events, preserves Google's per-event attendee notifications, and sends one consolidated Mission Control email summarizing old and new session times. Session and cohort cancellation actions must use a confirmation dialog, notify Google attendees, and reset readiness without deleting delivery history.
