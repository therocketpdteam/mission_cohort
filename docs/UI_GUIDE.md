...
- Publish readiness must explain the blocker, not only count it. Show compact session-level detail for calendar status, required email coverage, stale schedule warnings after session changes, materials readiness, and the exact next action.
- Publish readiness should use a command-center composition: four compact summary tiles, a subdued action rail, a larger session automation panel, a right-side open-task rail, and a readiness score card. Keep every control connected to a real action; do not add decorative or dead preview buttons.
