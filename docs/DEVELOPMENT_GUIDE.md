...
- Registration Communication Journey should be grouped by operational meaning: Needs Attention, Scheduled, Sent, Skipped, and Planned. Show recipient type, timing, delivery signals, and provider errors in compact rows instead of hiding skipped records or rendering raw event streams.
- Journey rows may expose safe operator actions: preview message content, resend to a resolved recipient, mark failed/bounced issues reviewed, cancel unsent records, and open the message in Communications. Never delete sent history or silently send to unresolved/multiple recipients from the registration drawer.
...
- The invoice PDF visual should follow the RocketPD invoice template: strong purple brand header, bill-to and invoice metadata body, readable item table, payment/notes section, and a bottom total band. Keep generated PDFs polished enough to send directly to a client without exporting to another design tool.
